...
const BottomNavigation = () => {
  ...
  return (
    <nav className="fixed bottom-0 w-full lg:hidden bg-[#E7E7E7] rounded-t-2xl  z-50">
      <ul className="w-full flex items-center gap-x justify-between ">
        {mobileNavigation.map((nav, index) => (
          <li key={index} className="relative py-3 px-6">
            <span
              className={`absolute h-[3px] left-4 bottom-0 w-10 bg-[#0062FF] rounded-tr-full rounded-full ${
